...
- **Commit edits fast — the 2-min live push can eat them.** dc499_refresh.js runs `git rebase --autostash` every cycle. If a file is edited while that's mid-flight, the autostash can fail to re-apply and the edit gets stranded in `git stash list` (one autostash entry per event; 128 had piled up by 2026-09-24). Recover with `git show stash@{N}:<file>`. Prefer doing edit + commit in one step. The old autostash entries are safe to review and clear someday.
